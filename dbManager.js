...
function _getTimeTable(callBack) {
    dbWrapper.wrapDbActions(wrappedDDbActions => {
        let timeTable = [];
        wrappedDDbActions
            .dbAll(sqlQueries.SELECT_SHOWS_WITH_HALLS, [])
            .then(rows => {
                rows.forEach(row => {
                    timeTable.push({
                        id: row.id,
...
function _getMovieChairs(movieId, callBack) {
    dbWrapper.wrapDbActions(wrappedDDbActions => {
        wrappedDDbActions
            .dbGet(sqlQueries.SELECT_HALL_FOR_SHOW, [movieId])
            .then(row => {
...
function _selectSeatsForHall(idHall, movieId, callBack) {
    dbWrapper.wrapDbActions(wrappedDDbActions => {
        let movieChairs = [];
        wrappedDDbActions
            .dbAll(sqlQueries.SELECT_SEATS_FOR_HALL, [idHall])
            .then(rows => {
                rows.forEach(row => {
                    movieChairs.push({
                        id: row.seatId,
                ...
                _promisifiedGetBookedChairs(movieId)
                    .then(seatsIds => {
                        seatsIds.forEach(seatId => {
                            movieChairs.find(
                                chair => chair.id == seatId
                            ).state = "booked";
                        });
    ...
    dbWrapper.wrapDbActions(wrappedDDbActions => {
        try {
            if (seatIds && Array.isArray(seatIds)) {
                let stmt = wrappedDDbActions.stmtPrepare(
                    sqlQueries.INSERT_INTO_BOOKING
                );
...
function _getBookedChairs(movieId, callBack) {
    dbWrapper.wrapDbActions(wrappedDDbActions => {
        wrappedDDbActions
            .dbAll(sqlQueries.SELECT_BOOKED_SEATS_FOR_SHOW, [movieId])
            .then(rows => {
                let bookedChairs = [];
...
const _promisifiedGetBookedChairs = utils.promisify(
    _getBookedChairs.bind(this)
);
const _promisifiedSelectSeatsForHall = utils.promisify(
    _selectSeatsForHall.bind(this)
);
